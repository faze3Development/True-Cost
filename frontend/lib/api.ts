...
export const fetchProperties = async (bounds: string) => {
  const response = await api.get<ApiPropertyResponse[]>(`/properties?bounds=${bounds}`);
  return response.data;
};

export const fetchUnitHistory = async (unitId: string, days = 90): Promise<UnitHistoryPoint[]> => {
  const response = await api.get<ApiUnitHistoryRecord[]>(`/units/${unitId}/history?days=${days}`);
...
export const fetchPropertyUnits = async (propertyId: string | number): Promise<ApiUnit[]> => {
  const response = await api.get<ApiUnitResponse[]>(`/properties/${propertyId}/units`);
